...
`consolidator` is multi-section + multi-source + full-column-union with **no dedupe** (every source row is preserved — nothing is lost). Each **section** has one or more source spreadsheets (each with its own picked tabs), a user-chosen **output spreadsheet URL**, and a user-typed **output tab name**. Engine builds a column registry across all picked tabs (case-insensitive header normalize, first-seen order, first-seen casing kept for output headers) and appends every non-empty source row to the output. Output is the **column union** — not a fixed 4-column schema — written to the section's output tab (clear A:ZZ + full overwrite each run). The output tab is auto-created with enough columns if missing, or its grid is grown via `appendDimension` if its current `columnCount` is short.

After the bulk write, three post-write passes run on the output tab: (1) **Email duplicates** painted via `repeatCell` (first occurrence light green, subsequent occurrences light red — same convention as duplicate-finder); (2) **Phone duplicates** same rule (column matched against the same alias list as duplicate-finder, values normalized by stripping non-digits); (3) **Renewal Date conditional formatting** (alias-matched column) — date cells are converted to real Sheets date values (numberValue + DATE format) and the same four-tier `TODAY()`-based conditional rules used by renewal-sync are installed (past/0-4d/5-14d/15-30d). Conditional rules persist in the spreadsheet so the proximity highlighting refreshes daily without re-running. All format work is sent as one batched `batchUpdate` in chunks of 50; partial-failure surfaces in `result.sections[i].error` rather than aborting.

Date helpers `parseFlexibleDate` and `toSheetsDateSerial` live in `src/lib/sheet-dates.ts` and are shared with renewal-sync. Multiple sections are configured in one form and run sequentially by `runConsolidatorBatch` — per-section failures are isolated. The page UI runs each section independently via its own "Run section" button (per-section result + error + loading state), with an optional "Run all" convenience for batch firing. Config schema in `_consolidator_config` is `[sectionId, name, outputUrl, outputTabName, sources]` with `sources` as a JSON array; the original single-section legacy schema (`[sourceUrl, sourceTabs]`) is detected by header sniff and migrated to one section on read.
