...
`consolidator` is multi-section + multi-source + full-column-union. Each **section** has one or more source spreadsheets (each with its own picked tabs), a user-chosen **output spreadsheet URL**, and a user-typed **output tab name**. Engine builds a column registry across all picked tabs (case-insensitive header normalize, first-seen order, first-seen casing kept for output headers), dedupes rows by lowercased Email across all sources, and merges duplicates with "first non-blank value per column wins" (the generalization of the old phone-wins rule). Output is the **column union** — not a fixed 4-column schema — written to the section's output tab (clear A:ZZ + full overwrite each run). The output tab is auto-created with enough columns if missing, or its grid is grown via `appendDimension` if its current `columnCount` is short. Multiple sections are configured in one form and run sequentially by `runConsolidatorBatch` — per-section failures are returned in `result.sections[i].error` rather than aborting the batch. Config schema in `_consolidator_config` is `[sectionId, name, outputUrl, outputTabName, sources]` with `sources` as a JSON array; the original single-section legacy schema (`[sourceUrl, sourceTabs]`) is detected by header sniff and migrated to one section on read.
