...
> {
  ...
  if (!session.masterSheetId) {
    return {
      ok: false,
      response: NextResponse.json(
        { error: "No master sheet configured. Set one from the Sheets page." },
        { status: 400 }
      ),
    };
  }
...
export async function POST(request: Request) {
  try {
    ...
    const { refreshToken, masterSheetId } = auth.ctx;
    const body = await request.json();
    const parsed = AddSheetSchema.parse(body);

    const spreadsheetId = extractSpreadsheetId(parsed.url);
    const existing = await getLinkedSheets(refreshToken, masterSheetId);
    ...
    for (const tab of parsed.tabs) {
      ...
      try {
        ...
        errors.push(
          `Cannot read "${tab}": ${err instanceof Error ? err.message : String(err)}`
        );
        continue;
      }
