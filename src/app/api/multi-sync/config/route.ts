// src/app/api/multi-sync/config/route.ts
...
import {
  getMultiSyncConfig,
  saveMultiSyncConfig,
  clearMultiSyncConfig,
} from "@/lib/multi-sync-config-store";
...
export async function GET() {
  ...
  try {
    const config = await getMultiSyncConfig(
...
export async function DELETE() {
  ...
  if ("error" in session) return session.error;
  try {
    await clearMultiSyncConfig(session.refreshToken, session.masterSheetId);
    return NextResponse.json({ ok: true });
  } catch (err) {
    return NextResponse.json(
