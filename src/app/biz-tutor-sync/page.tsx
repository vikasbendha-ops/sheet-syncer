...
export default function BizTutorSyncPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch("/api/biz-tutor-sync/config");
        if (res.status === 400) {
          ...
          if (data.sourceUrl) {
            setHasSaved(true);
            promises.push(
              fetchTabsForUrl(data.sourceUrl, data.sourceTabs, setSource)
            );
          }
          if (data.lookupUrl) {
            setHasSaved(true);
            promises.push(
              fetchTabsForUrl(data.lookupUrl, data.lookupTabs, setLookup)
            );
          }
  ...
  useEffect(() => {
    ...
    const hasAnything =
      ...
      lookup.selected.length;
    if (!hasAnything) return;

    const handle = setTimeout(() => {
      fetch("/api/biz-tutor-sync/config", {
        method: "PUT",
  ...
  function fetchTabs(state: SheetState, setState: (s: SheetState) => void) {
    if (!state.url) return;
    setResult(null);
    fetchTabsForUrl(state.url, null, setState);
  }
  ...
  async function handleClearSaved() {
    setError("");
    try {
      await fetch("/api/biz-tutor-sync/config", { method: "DELETE" });
    } catch {
      // best-effort — still reset UI
    }
    setSource(initialSheet);
    setLookup(initialSheet);
    setResult(null);
    setHasSaved(false);
  }
