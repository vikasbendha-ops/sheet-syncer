...
async function ensureConfigTab(): Promise<void> {
...
export async function getLinkedSheets(): Promise<LinkedSheet[]> {
...
export async function removeLinkedSheet(index: number): Promise<void> {
  await ensureConfigTab();
  const sheets = getSheetsClient();
  const masterSheetId = getMasterSheetId();

  // Read all rows to rebuild without the target row
  const response = await sheets.spreadsheets.values.get({
    spreadsheetId: masterSheetId,
    range: `${CONFIG_TAB}!A2:D1000`,
  });
