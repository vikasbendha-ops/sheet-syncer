...
export async function runFullSync(refreshToken: string): Promise<SyncResult> {
  ...
  const crossRef = new Map<
    string,
    Array<{ nickname: string; url: string; sheetKey: string }>
  >();
  for (const sheet of sheetEmails) {
    for (const email of sheet.emails.keys()) {
      ...
      list.push({ nickname: sheet.nickname, url: sheet.url, sheetKey: sheet.key });
    }
  }
    ...
    try {
      ...
      for (const [email, rowNumbers] of sheet.emails.entries()) {
        ...
        for (const rowNumber of rowNumbers) {
          cellData.push({
            rowIndex: rowNumber - 1, // convert 1-based to 0-based
            links: others.map((o) => ({ text: o.nickname, url: o.url })),
          });
        }
      }
