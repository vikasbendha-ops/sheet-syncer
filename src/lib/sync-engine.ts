...
export async function runFullSync(
  ...
  // Merge names across source sheets: for each email, count all name occurrences
  // across every sheet and pick the most common. Ties go to the name first seen
  // (which follows config order since sheetEmails is sorted by index).
  const mergedNames = new Map<string, string>();
  for (const email of allEmails) {
    const counts = new Map<string, number>(); // lowercased key → count
    const originalCase = new Map<string, string>(); // lowercased key → first-seen casing
    ...
    for (const sheet of sheetEmails) {
      ...
      for (const name of list) {
        ...
        if (!originalCase.has(key)) originalCase.set(key, name);
      }
    }
    ...
    const chosen = originalCase.get(bestKey);
    if (chosen) mergedNames.set(email, chosen);
  }
