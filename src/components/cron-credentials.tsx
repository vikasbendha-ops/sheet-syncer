    ...
    <div className="bg-card border border-border rounded-lg p-5 space-y-3">
      <div>
        ...
        <p className="text-sm text-muted mt-1">
          To run automated syncs every 30 minutes, set these env vars on your host
          and configure a cron to call <code className="bg-background px-1 py-0.5 rounded text-xs">/api/cron</code>.
        </p>
      </div>
      ...
      {!show ? (
        ...
        <div className="space-y-3">
          ...
          <p className="text-xs text-muted">
            Also set <code className="bg-background px-1 py-0.5 rounded">CRON_SECRET</code> to a
            random string. Then schedule a cron to hit{" "}
            <code className="bg-background px-1 py-0.5 rounded break-all">
              https://your-domain/api/cron?key=&lt;CRON_SECRET&gt;
            </code>{" "}
            every 30 minutes (cron expression: <code className="bg-background px-1 py-0.5 rounded">*/30 * * * *</code>).
          </p>
          <button
